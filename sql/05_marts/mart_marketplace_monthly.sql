...
WITH monthly AS (
 SELECT DATE_TRUNC('month', order_purchase_timestamp)::date AS month_start, COUNT(DISTINCT order_id) AS orders, SUM(gmv_proxy) AS gmv_proxy, COUNT(DISTINCT seller_id) AS active_sellers
