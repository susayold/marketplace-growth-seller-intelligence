...
def main() -> None:
    ...
    trigger3 = "Median first-sale time is {:.0f} days and activation by 30 days is {:.1%} among valid links.".format(float(activation.median_days_to_first_sale), float(activation.activation_rate_le_30d))
    add("RC3", trigger3, "H1 seller onboarding has a long time-to-value", "Activation timing distribution and cumulative curve show a delayed first sale.", "Historical data does not identify which onboarding step caused delay.", "descriptive timing evidence", "only {:.1%} activate by 30 days".format(float(activation.activation_rate_le_30d)), "seller readiness, category and demand differences", "Supported business signal", "instrument 7/30/60-day onboarding checkpoints")
