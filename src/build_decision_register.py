...
def main() -> None:
    ...
    cards = [
        ...
        {"decision_id": "D03", "problem": "Seller time-to-value is slow after conversion.", "finding": "Canonical v3 estimates {:.1%} activated within 30 observable days; observed activator median first sale is {:.1f} days.".format(float(act.activation_rate_le_30d), float(act.median_activation_days)), "evidence_level": "LEVEL 1 — descriptive timing signal", "confidence": "MEDIUM", "target_scope": "activation-eligible closed sellers", "recommended_action": "Instrument onboarding checkpoints at 7, 30 and 60 days and test interventions rather than assuming a causal fix.", "why_this_action": "The delay is large enough to monitor; the intervention should be evaluated against retention and downstream value.", "primary_kpi": "v3 30/60-day activation and observed activator median days-to-first-sale", "guardrail_kpi": "M3 retention and downstream GMV", "owner_role": "Seller Operations", "review_frequency": "weekly", "stop_condition": "stop or redesign an onboarding change if activation improves while retention/value deteriorates", "limitation": "Seller readiness, category mix and demand are not fully observed; fixed-window rates use observable denominators.", "evidence_strength": 2, "actionability": 2, "scope_size": "840 activation-eligible sellers; 825 observable at 30 days", "priority_band": "P1"},
